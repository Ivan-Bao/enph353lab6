...
if __name__ == '__main__':
    ...
    for x in range(total_episodes):
        ...
        # if (x > 0) and (x % 5 == 0): # MODIFY THIS! Make it so it saves the highest reward
        #     qlearn.saveQ("QValues")
        #     plotter.plot(env)
        if cumulated_reward > prevEpHighestReward: # MODIFY THIS! Make it so it saves the highest reward
            qlearn.saveQ("QValues")
            plotter.plot(env)
        prevEpHighestReward = cumulated_reward
