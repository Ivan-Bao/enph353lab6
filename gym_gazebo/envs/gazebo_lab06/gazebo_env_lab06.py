...
class Gazebo_Lab06_Env(gazebo_env.GazeboEnv):
    ...
    def process_image(self, data):
        ...
        # bottompart = cv_image[cv_image.shape[0] - 100:cv_image.shape[0], :, :]  # Cuts the video and focuses on the bottom part
        #
        # bottomGrey = cv2.cvtColor(bottompart, cv2.COLOR_BGR2GRAY)  # Convert to grey scale image
        #
        # (thresh, bottomBIN) = cv2.threshold(bottomGrey, 127, 255, cv2.THRESH_BINARY)  # Convert to binary image
        # bottomBINInv = np.invert(bottomBIN)  # and invert the color to find center of mass of the image
        # mass = np.sum(bottomBINInv)
        # if mass < 25500:
        #     self.timeout = self.timeout+1
        #     if self.timeout >= 30:
        #         done = True
        # else:
        #     (cy, cx) = ndi.measurements.center_of_mass(bottomBINInv)  # Finds the center of mass using SciPy
        #     position = math.floor(10*cx/cv_image.shape[1])
        #     cv2.circle(cv_image, (int(cx), int(cy) + (cv_image.shape[0] - 100)), 20, (0, 0, 255),
        #                -1)  # Draw a circle as a CoM mark on the original frame
        #     state[position] = 1
        #     self.timeout = 0        # font = cv2.FONT_HERSHEY_SIMPLEX
        # cv2.imshow("img", cv_image)
        bottompart = cv_image[240 - 50:240, :, :]  # Cuts the video and focuses on the bottom part
        ...
        if mass < 25500:
            ...
            (cy, cx) = ndi.measurements.center_of_mass(bottomBINInv)  # Finds the center of mass using SciPy
            position = int(math.floor(10*cx/320))
            cv2.circle(cv_image, (int(cx), int(cy) + (240 - 50)), 20, (0, 0, 255),
                       -1)  # Draw a circle as a CoM mark on the original frame
            state[position] = 1
            self.timeout = 0     
        ...
        # topLeftCornerOfText = (10, cv_image.shape[0]-15)
        # fontScale = 1
        # fontColor = (255, 255, 255)
        # lineType = 2
        #
        # cv2.putText(cv_image, str(state),
        #             topLeftCornerOfText,
        #             font,
        #             fontScale,
        #             fontColor,
